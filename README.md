...
1) ``` git clone https://github.com/opklopper-team/heirloom-pantry-club.git . ```  
...
``` shopify theme dev --live-reload=off -s solarsilver.myshopify.com ```
...
1) ``` shopify theme dev --live-reload=off -s heirloom-pantry-club.myshopify.com ``` - for watch scss and js files in DEV mode (with sudo in unix systems);
